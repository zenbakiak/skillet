...
pub fn exec_reduce(
    ...
    let lambda_expr = &args_expr[0];
    let mut accumulator = eval_with_vars(&args_expr[1], &base_vars.cloned().unwrap_or_default())?;
...
pub fn exec_reduce_with_custom(
    ...
    let lambda_expr = &args_expr[0];
    let mut accumulator = eval_with_vars_and_custom(&args_expr[1], &base_vars.cloned().unwrap_or_default(), custom_registry)?;
